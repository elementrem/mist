...
window.addEventListener('message', function message(event) {
    ...
    // ElementremProvider: connect
    if (data.type === 'create') {
        ipcRenderer.send('ipcProvider-create');

    // ElementremProvider: write
...
const elementremProvider = fs.readFileSync(path.join(__dirname, '/injected/ElementremProvider.js')).toString();
