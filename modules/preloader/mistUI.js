...
// also certainly remove the web3.elebase one
...
ipcRenderer.on('uiAction_runTests', (e, type) => {
    if (type === 'webview') {
        web3.ele.getAccounts((error, accounts) => {
            if (error) return;

            web3.ele.getCoinbase((coinbaseError, coinbase) => {
                ...
                Tracker.autorun(() => {
                    const accountList = _.pluck(EleAccounts.find({}, { fields: { address: 1 } }).fetch(), 'address');
