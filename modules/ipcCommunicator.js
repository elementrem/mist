...
ipc.on('backendAction_importPresaleFile', function(e, path, pw) {
    ...
    var nodeProcess = spawn(getNodePath('gele'), ['wallet', 'import', path]);

    nodeProcess.once('error',function(){
        error = true;
        e.sender.send('uiAction_importedPresaleFile', 'Couldn\'t start the "gele wallet import <file.json>" process.');
    });
