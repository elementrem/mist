...
// should be       'https://raw.githubusercontent.com/elementrem/mist/master/clientBinaries.json'
const BINARY_URL = 'https://raw.githubusercontent.com/elementrem/mist/master/clientBinaries.json';

const ALLOWED_DOWNLOAD_URLS_REGEX =
    /^https:\/\/github.com\/.+$/;  // eslint-disable-line max-len
...
class Manager extends EventEmitter {
    ...
    _checkForNewConfig(restart) {
        ...
        .then((localConfig) => {
            ...
            return mgr.init({
                folders: [
                    path.join(Settings.userDataPath, 'binaries', 'Gele', 'unpacked'),
                    path.join(Settings.userDataPath, 'binaries', 'Ele', 'unpacked'),
                ],
            })
            .then(() => {
    ...
    _resolveEleBinPath() {
        log.info('Resolving path to Ele client binary ...');
        ...
        binPath = path.join(path.resolve(binPath), 'ele');

        if (platform === 'win') {
            binPath += '.exe';
        }

        log.info(`Ele client binary path: ${binPath}`);

        this._availableClients.ele = {
            binPath,
            version: '1.3.0',
        };
    }
