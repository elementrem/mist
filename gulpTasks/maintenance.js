...
gulp.task('update-nodes', (cb) => {
    const clientBinariesGele = clientBinaries.clients.Gele;
    const localGeleVersion = clientBinariesGele.version;
    const newJson = clientBinaries;
    const gele = newJson.clients.Gele;

    // Query latest gele version
    got('https://api.github.com/repos/elementrem/go-elementrem/releases/latest', { json: true })
    .then((response) => {
        return response.body.tag_name;
    })
    // Return tag name (e.g. 'v1.5.0')
    .then((tagName) => {
        const latestGeleVersion = tagName.match(/\d+\.\d+\.\d+/)[0];

        // Compare to current gele version in clientBinaries.json
        if (cmp(latestGeleVersion, localGeleVersion)) {
            gele.version = latestGeleVersion;

            // Query commit hash (first 8 characters)
            got(`https://api.github.com/repos/elementrem/go-elementrem/commits/${tagName}`, { json: true })
            .then((response) => {
            ...
            .then((hash) => {
                ...
                got('https://github.io/elementrem/builds?restype=container&comp=list', { xml: true })
                .then((response) => {
                    ...
                    _.keys(gele.platforms).forEach((platform) => {
                        _.keys(gele.platforms[platform]).forEach((arch) => {
                            // Update URL
                            let url = gele.platforms[platform][arch].download.url;
                            url = url.replace(/\d+\.\d+\.\d+-[a-z0-9]{8}/, `${latestGeleVersion}-${hash}`);
                            gele.platforms[platform][arch].download.url = url;

                            // Update bin name (path in archive)
                            let bin = gele.platforms[platform][arch].download.bin;
                            bin = bin.replace(/\d+\.\d+\.\d+-[a-z0-9]{8}/, `${latestGeleVersion}-${hash}`);
                            gele.platforms[platform][arch].download.bin = bin;

                            // Update expected sanity-command version output
                            gele.platforms[platform][arch].commands.sanity.output[1] =
                            String(latestGeleVersion);

                            // Update md5 checksum
                            blobs.forEach((blob) => {
                                if (String(blob.Name) === _.last(gele.platforms[platform][arch].download.url.split('/'))) {
                                    const sum = new Buffer(blob.Properties[0]['Content-MD5'][0], 'base64');

                                    gele.platforms[platform][arch].download.md5 = sum.toString('hex');
                                }
                            });
                        });
                    });
