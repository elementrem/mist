...
test['Browser bar should not render script tags on breadcrumb view'] = function* () { // ELE-01-001
...
test['Browser bar should not render script tags in disguise on breadcrumb view'] = function* () { // ELE-01-001
    const client = this.client;
...
test['Browser bar should not render script tags in disguise (2) on breadcrumb view'] = function* () { // ELE-01-001
...
test['Browser bar should not render arbitrary code as HTML'] = function* () { // ELE-01-001
    const client = this.client;
...
test['Browser bar should not execute JS'] = function* () { // ELE-01-001
    const client = this.client;
...
test['"http" protocol should be allowed on browser bar'] = function* () { // ELE-01-002
    const client = this.client;
    yield this.loadFixture();
...
test['"javascript" protocol should be disallowed on browser bar'] = function* () { // ELE-01-002
    const client = this.client;
    yield this.loadFixture();
...
test['"data" protocol should be disallowed on browser bar'] = function* () { // ELE-01-002
    const client = this.client;
    yield this.loadFixture();
...
test['"file" protocol should be disallowed on browser bar'] = function* () { // ELE-01-012
...
test['Browse tab should be changed to pinned tab if URLs are the same'] = function* () { // ELE-01-007
    const client = this.client;
    yield this.selectTab('browser');

    yield this.navigateTo('https://wallet.elementrem.org');
    yield Q.delay(1000);
...
test['Wallet tab shouldn\'t have the page replaced if URLs does not match'] = function* () { // ELE-01-007
    const client = this.client;
    yield this.selectTab('wallet');

    yield this.navigateTo(`${this.fixtureBaseUrl}index.html?https://wallet.elementrem.org`);
    yield client.waitUntil(() => {
...
test['Wallet tab shouldn\'t have the page replaced if URLs does not match - 2'] = function* () { // ELE-01-007
    const client = this.client;
    yield this.selectTab('wallet');

    // Now changing address via JS
    yield client.setValue('#url-input', `${this.fixtureBaseUrl}index.html?https://wallet.elementrem.org`);
