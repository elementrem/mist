...
describe('Permissions', function () {
    ...
    describe('web3.ele.accounts', function () {

        it('should return an array [sync]', function () {
            var accounts = web3.ele.accounts;
            expect(accounts).to.be.an('array');
        });

        it('should return an array [async]', function (done) {
            web3.ele.getAccounts(function (e, accounts) {
                expect(e).to.be.null;
                expect(accounts).to.be.an('array');
        ...
        it('should match the allowed accounts in the tabs permisssions, and don\'t contain coinbase [async, batch request]', function (done) {
            ...
            batch.add(web3.ele.getAccounts.request(callback));
            batch.add(web3.ele.getAccounts.request(callback));
            batch.execute();
        });
    ...
    describe('web3.ele.coinbase', function () {
        ...
        it('should be empty [async]', function (done) {
            web3.ele.getCoinbase(function (e, coinbase) {
                expect(e).to.be.null;
                expect(coinbase).to.be.null;
        ...
        it('should be empty [async, batch request]', function (done) {
            ...
            var batch = web3.createBatch();
            batch.add(web3.ele.getCoinbase.request(callback));
            batch.add(web3.ele.getCoinbase.request(callback));
            batch.execute();
        });
    ...
    describe('web3 parameter validation', function () {
        ...
        it('shouldn\'t allow RegExp (possible XSS)', function () {
            ...
            expect(function () { web3.ele.sendTransaction({ from: add, to: add, data: new RegExp('') }); }).to.throw('Payload, or some of its content properties are invalid. Please check if they are valid HEX with \'0x\' prefix.');
        });
    });
