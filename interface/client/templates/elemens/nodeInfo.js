...
var getMining = function(template) {
    web3.ele.getMining(function(e, res) {
        if(!e && res) {
            web3.ele.getHashrate(function(e, res) {
                if(!e) {
...
@class [template] elemens_nodeInfo
@constructor
*/

Template['elemens_nodeInfo'].onCreated(function(){
    var template = this;
    ...
    // CHECK SYNCING
    this.syncFilter = web3.ele.isSyncing(function(error, syncing) {
        if(!error) {
...
Template['elemens_nodeInfo'].onDestroyed(function(){
    Meteor.clearInterval(this.peerCountIntervalId);
...
Template['elemens_nodeInfo'].helpers({
    /**
    ...
    */
    'formattedBlockNumber': function() {
        return numeral(EleBlocks.latest.number).format('0,0');
    },
    /**
    ...
    */
    'timeSinceBlock': function () {
        var timeSince = moment(EleBlocks.latest.timestamp, "X");
        var now = moment();
