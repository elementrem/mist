...
onReady = () => {
    ...
    const kickStart = () => {
        ...
        elementremNode.on('nodeConnectionTimeout', () => {
            Windows.broadcast('uiAction_nodeStatus', 'connectionTimeout');
        });

        elementremNode.on('nodeLog', (data) => {
            Windows.broadcast('uiAction_nodeLogText', data.replace(/^.*[0-9]]/, ''));
        });

        // state change
        elementremNode.on('state', (state, stateAsText) => {
        ...
        Q.try(() => {
            ...
            if ((Settings.loadUserData('daoFork') || '').trim() === 'false') {
                dialog.showMessageBox({
                    ...
                    shell.openExternal('https://github.com/elementrem/mist/releases');
                    app.quit();
                });
...
/*
            if (elementremNode.isGele && (resultData.result === null || (_.isArray(resultData.result) && resultData.result.length === 0))) {
                ...
                return new Q((resolve, reject) => {
                    ...
                    ipcMain.on('onBoarding_changeNet', (e, testnet) => {
                        const newType = elementremNode.type;
