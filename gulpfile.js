...
var geleVersion = '1.4.14';
var nodeUrls = {
    'mac-x64': {
        url: 'https://github.com/elementrem/go-elementrem/releases/download/v1.4.14/mac-osx-gele-1.4.14-47d51ec.zip',
        ext: 'zip'
    },
    'linux-x64': {
        url: 'https://github.com/elementrem/go-elementrem/releases/download/v1.4.14/linux-64bit-gele-1.4.14-47d51ec.zip',
        ext: 'zip',
    },
    'win-x64': {
        url: 'https://github.com/elementrem/go-elementrem/releases/download/v1.4.14/windows-64bit-gele-1.4.14-47d51ec.zip',
        ext: 'zip',
    },
};
...
gulp.task('downloadNodes', ['clean:nodes'], function() {
    ...
    _.each(nodeUrls, function(info, osArch) {
        ...
        if (platformIsActive(osArch)) {
            toDownload.push({
                file: `gele_${geleVersion}_${osArch}.${info.ext}`,
                url: info.url,
            });
        }
    });
...
gulp.task('renameNodes', ['unzipNodes'], function(done) {
    ...
    for (let osArch in nodeUrls) {
        ...
        const finalName = (0 <= osArch.indexOf('win') ? 'gele.exe' : 'gele');
...
gulp.task('checkNodes', function(cb) {
    _.each(nodeUrls, (info, osArch) => {
        // check for zip file
        if (platformIsActive(osArch)) {
            const fileName = `gele_${geleVersion}_${osArch}.${info.ext}`;

            try {
                fs.accessSync(`./nodes/gele/${fileName}`, fs.R_OK);
            } catch (err) {
    ...
    _.each(nodeUrls, (info, osArch) => {
        if (platformIsActive(osArch)) {
            ...
            streams.push(gulp.src([
                ...
                .pipe(gulp.dest('./dist_'+ type +'/app/nodes/ele/' + osArch)));

            // copy gele node binaries
            streams.push(gulp.src([
                './nodes/gele/'+ osArch + '/*'
            ])
                .pipe(gulp.dest('./dist_'+ type +'/app/nodes/gele/' + osArch)));
        }
    });
